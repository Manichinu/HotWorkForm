...
export default class HotWorkViewForm extends React.Component<IHotWorkProps, HotWorkState, {}> {
    ...
    public getWorkflowHistory() {
        NewWeb.lists.getByTitle("HotWork Workflow History").items.filter(`RequestID eq '${SessionID}'`).orderBy("Id", false).get().then((items) => {
            if (items.length != 0) {
                console.log(items)
                for (var i = 0; i < items.length; i++) {
                    if (i == 0) {
                        ...
                        $(".section" + items[i].Level + " .button").hide();
                    }
                }
            } else {
                $('input, input[type="radio"],textarea,button').prop({ readonly: true, disabled: true });
    ...
    public getWorkflowDetails() {
        NewWeb.lists.getByTitle("HotWork Workflow History").items.select("*", "AssignedTo/EMail", "AssignedTo/Title", "AssignedBy/EMail", "AssignedBy/Title", "ApprovedBy/EMail", "ApprovedBy/Title").expand("AssignedTo", "ApprovedBy", "AssignedBy").filter(`RequestID eq '${SessionID}'`).orderBy("Id", true).get().then((items) => {
            if (items.length != 0) {
                console.log(items)
                for (var i = 0; i < items.length; i++) {
                    if (items[i].Status == "Approved") {
                        $(".tab" + items[i].Level + " .pending").removeClass('active');
                        $(".tab" + items[i].Level + " .completed").addClass('active');
                    }
                }
                this.setState({
                    WFDetails: items
    ...
    public setCurrentSecForm(level: number) {
        this.setState({
            currentPage: level
        })
    }
